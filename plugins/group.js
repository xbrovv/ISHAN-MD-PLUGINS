...
async (ishan, mek, m, { from, isGroup, isAdmins, reply }) => {
    try {
        if (!isGroup) return reply("⚠️ This command can only be used in a group!");
        if (!isAdmins) return reply("⚠️ This command is only for group admins!");

        await ishan.groupSettingUpdate(from, "not_announcement");
...
async (ishan, mek, m, { from, isGroup, isAdmins, reply }) => {
    try {
        ...
        await ishan.groupSettingUpdate(from, "announcement");
...
cmd({
  ...
  category: "group",
  filename: __filename,
}, async (ishan, mek, m, { isGroup, isAdmins, reply }) => {
  if (!isGroup || !isAdmins) 
    return reply("*Group only & both you and I must be admins.*");

  await ishan.groupRevokeInvite(m.chat);
...
cmd({
  ...
  category: "group",
  filename: __filename,
}, async (ishan, mek, m, { isGroup, reply }) => {
  if (!isGroup) 
    return reply("*Group only & I must be an admin.*");

  const code = await ishan.groupInviteCode(m.chat);
...
cmd({
  ...
  category: "group",
  filename: __filename,
}, async (ishan, mek, m, { isGroup, isAdmins, args, reply }) => {
  if (!isGroup || !isAdmins) 
    return reply("*Group only & both you and I must be admins.*");

  if (!args[0]) return reply("*Give a new group name.*");

  await ishan.groupUpdateSubject(m.chat, args.join(" "));
...
cmd({
  ...
  await ishan.groupUpdateDescription(m.chat, args.join(" "));
...
cmd({
  ...
  const metadata = await ishan.groupMetadata(m.chat);
