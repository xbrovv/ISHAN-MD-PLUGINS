...
cmd({
  ...
  try {
    if (!q) {
      return reply("*‼️ Please provide a valid MediaFire link.*");
    }
    ...
    if (!data || !data.status || !data.result || !data.result.dl_link) {
      return reply("*⚠️ Invalid or private MediaFire link.*");
    }
    ...
    await conn.sendMessage(from, {
      ...
      mimetype: mime_type,
      fileName: file_name,
      caption: `> *Powered by Ishan Madusanke🎐*`
    }, { quoted: m });
...
cmd({
  ...
  try {
    if (!q) {
      return reply("*🏷️ Please provide a valid Google Drive link.*");
    }
